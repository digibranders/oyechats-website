...
export const BLOG_POSTS: BlogPost[] = [
  {
    ...
    content: [
      ...
      {
        type: 'p',
        text: 'An AI chatbot changes the economics of collection. When a visitor is already in a natural conversation about your product, BANT signals leak out on their own. The job of the bot is to notice them, structure them, and put them in front of sales before the tab is closed.',
      },
      { type: 'h2', text: 'What each BANT signal looks like in a real chat' },
      {
        type: 'p',
        text: 'OyeChats treats each of the four dimensions as a probability, not a boolean. Every visitor message is scored against a small classifier that watches for language patterns tied to that dimension. A single confident sentence can move the score. A vague one nudges it.',
      },
      { type: 'h3', text: 'Budget' },
      {
        type: 'p',
        text: 'Visitors rarely name a number. They name a shape. "We are a five person team", "we are on the free plan of a competitor", or "we have a small ops budget for tools this quarter" all imply a range. The bot maps that range to a plan tier and stores the raw phrase for the rep to read later.',
      },
      { type: 'h3', text: 'Authority' },
      {
        type: 'p',
        text: 'A visitor who says "I would need to check with our head of support" is telling you exactly where they sit in the org. So is one who says "I run growth here". OyeChats tags the role, the pronoun pattern, and whether they speak in "we" or "I" when discussing purchase decisions.',
      },
      { type: 'h3', text: 'Need' },
      {
        type: 'p',
        text: 'Need is the easiest of the four because it is the reason the visitor started chatting. The trick is separating a real pain from a feature curiosity. A person who asks "does this integrate with HubSpot?" is exploring. A person who says "our current tool is dropping tickets after eight in the evening" is bleeding.',
      },
      { type: 'h3', text: 'Timing' },
      {
        type: 'p',
        text: 'Timing surfaces through deadlines, launch dates, and frustration intensity. Phrases like "we are rolling this out next month" or "we need to fix this before Black Friday" push the timing score sharply. So does a visitor returning three times in a week.',
      },
      { type: 'h2', text: 'Turning four numbers into a single decision' },
      {
        type: 'p',
        text: 'Once every score has a value between zero and one, OyeChats blends them into a lead grade. High Need with high Timing but unknown Budget still deserves a call, because the sales team can qualify the number faster than a bot can. High Budget with weak Need is a nurture, not a call. The exact weighting is tunable per account.',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'A form gets you the data of the small percentage of visitors willing to fill it. A conversation gets you the shape of intent from everyone who talks. Even a visitor who never becomes a lead has told you something about the market you are selling into, and that is worth keeping.',
      },
      {
        type: 'p',
        text: 'BANT was never broken. It was just waiting for a collection method that felt like a normal exchange between two humans. That is what an AI chatbot finally does at scale.',
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'p',
        text: 'Almost every team that picks up a large language model for the first time ends up asking the same question. Should we fine tune it on our data, or should we build a retrieval pipeline around it? The two paths look similar from a distance because they both promise the same outcome, which is answers that sound like your company wrote them. Up close they solve different problems.',
      },
      { type: 'h2', text: 'The one sentence version' },
      {
        type: 'p',
        text: 'Fine tuning changes how the model speaks. Retrieval changes what the model knows in the moment. If your problem is tone, format, or a repeatable pattern, fine tune. If your problem is up to date facts about your product, your policies, or your customers, retrieve.',
      },
      { type: 'h2', text: 'Where RAG wins outright' },
      {
        type: 'p',
        text: 'Any body of knowledge that changes weekly, monthly, or unpredictably belongs in retrieval. Documentation, pricing, feature flags, help center articles, and internal wikis all fall into this bucket. A retrieval pipeline lets you update a single source of truth and see the answer change on the next question.',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'Once a team is running in the real world, the split usually resolves the same way. Retrieval provides the facts. A lightly fine tuned model provides the personality and the output shape. The base model reads a small pack of relevant chunks and answers in the voice you trained it in. This is how OyeChats runs by default. Documents are chunked, embedded, and searched with a hybrid pipeline, and the response layer is prompted to sound consistent across every bot.',
      },
      ...
      {
        type: 'p',
        text: 'Neither approach is inherently more advanced than the other. They are two different tools that people keep reaching for at the wrong moment. Pick the one that maps to the failure you actually have today, and leave the other on the shelf until you need it.',
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'p',
        text: 'Vector search gets a lot of the press in modern retrieval, and for good reason. It handles paraphrasing, synonyms, and messy human phrasing in a way that plain keyword search never could. But if you have ever watched a visitor ask a question that includes an internal product code, a version number, or an acronym, you have also seen vector search miss badly. Hybrid search exists because both methods have blind spots, and their blind spots barely overlap.',
      },
      { type: 'h2', text: 'What each method is actually good at' },
      {
        type: 'p',
        text: 'Vector search compares meaning. It turns text into a dense numerical representation and looks for chunks whose meaning is close to the query. That is exactly what you want when a visitor asks about "cancelling my plan" and the docs use the phrase "ending your subscription".',
      },
      {
        type: 'p',
        text: 'Keyword search, in the form of BM25 or a Postgres TSVECTOR index, compares literal tokens. It is exactly what you want when a visitor pastes the string "ERR_TIMEOUT_1044" and needs the doc that mentions that exact code. No embedding model is going to reliably guess that a specific error code should map to a specific troubleshooting page.',
      },
      { type: 'h2', text: 'How to combine the two' },
      {
        type: 'p',
        text: 'The simplest and most reliable technique is reciprocal rank fusion. Run both searches, get two ranked lists, and combine them by rank rather than by raw score. A chunk that shows up near the top of either list gets credit. A chunk that shows up near the top of both lists gets a lot of credit.',
      },
      {
        type: 'code',
        lang: 'python',
        text: 'def rrf(vector_hits, keyword_hits, k=60):\n    scores = {}\n    for rank, doc_id in enumerate(vector_hits):\n        scores[doc_id] = scores.get(doc_id, 0) + 1 / (k + rank)\n    for rank, doc_id in enumerate(keyword_hits):\n        scores[doc_id] = scores.get(doc_id, 0) + 1 / (k + rank)\n    return sorted(scores.items(), key=lambda x: x[1], reverse=True)',
      },
      {
        type: 'p',
        text: 'The k parameter is a smoothing constant. A value around 60 works well in practice. Larger values flatten the contribution of top ranked results. Smaller values sharpen it.',
      },
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: 'p',
        text: 'Behavioural tracking earns its keep when it makes the conversation smarter. If it is only feeding a report, you can probably delete half of the tags and lose nothing.',
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'p',
        text: 'Webhooks look simple on a whiteboard. Something happens on our side, we send a POST to your URL. In production, the same feature is where roughly a third of integration bugs quietly live. Retry storms, delivery gaps, silent signature mismatches, and endpoints that return two hundred OK while doing nothing at all. The rules below are what we have learned running webhooks across thousands of bots.',
      },
      { type: 'h2', text: 'Sign every payload, and pin the signature scheme' },
      {
        type: 'p',
        text: 'Every webhook we send carries an HMAC SHA 256 signature over the raw request body, computed with a secret unique to the receiver. Two things matter here. First, sign the raw bytes, not the parsed JSON, because any JSON reserialisation on either side breaks verification. Second, put the scheme identifier in the header, so future rotations do not require a coordinated release.',
      },
      {
        type: 'code',
        lang: 'http',
        text: 'X-OyeChats-Signature: v1=8f2a1c93b4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1\nX-OyeChats-Timestamp: 1739299200',
      },
      {
        type: 'p',
        text: 'The timestamp is not decoration. Verifiers should reject any request more than five minutes old, which kills replay attacks and also catches misconfigured servers whose clocks have drifted.',
      },
      { type: 'h2', text: 'Retry with exponential backoff and a hard cap' },
      {
        type: 'p',
        text: 'If your receiver returns anything other than a two hundred status, we retry. Retries follow exponential backoff with jitter, and we cap the total window at twenty four hours. After that we mark the delivery as failed and expose it in the admin dashboard for manual replay. This bounds the blast radius when a downstream partner has an incident.',
      },
      {
        type: 'ul',
        items: [
      ...
      {
        type: 'p',
        text: 'The sender does not know whether your two hundred came before or after the crash. The only safe assumption is that any event might arrive twice. Every event carries a stable delivery id and a stable event id. Store the event id on the receiver, check it before applying side effects, and skip if it has already been processed. Idempotency is the difference between a webhook that works and a webhook that occasionally duplicates every message in a live chat.',
      },
      { type: 'h2', text: 'Send small, coherent events' },
      {
        type: 'p',
        text: 'Large batched payloads are tempting because they cut request volume. They also make retries expensive, encourage partial failure handling that nobody actually writes, and force the receiver to parse events they may not care about. We prefer one event per business fact. A new chat session, a new message, a status change. The receiver filters what it wants.',
      },
      { type: 'h2', text: 'Observability the receiver can trust' },
      {
        type: 'p',
        text: 'Every webhook we send is inspectable in the admin dashboard. The receiver sees the raw payload, the response status, the response body, the delivery timestamps, and every retry attempt with its reason. When integrations break at three in the morning, that is the difference between a five minute fix and a two hour support thread.',
      },
      {
        type: 'p',
        text: 'None of this is exotic. It is a small collection of habits that pay off the first time a downstream system has a bad day, which will happen sooner than you expect.',
      },
    ],
  },
];
