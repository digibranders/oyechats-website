...
const socials: Array<{ label: string; icon: React.ReactNode }> = [
  {
    label: 'Twitter / X',
    icon: (
            ...
            <div className="flex items-center gap-2.5">
              {socials.map((social) => (
                <span key={social.label}
                  ...
                  title="Coming soon"
                  className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/30">
                  {social.icon}
                </span>
              ))}
            </div>
