        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="glass-2 rounded-3xl border border-white/10 p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              {METRICS.map((m) => (
                <div key={m.label} className="glass-1 rounded-xl p-4 border border-white/8">
                  <p className="text-[11px] text-white/70 mb-1">{m.label}</p>
            ...
            <div className="glass-1 rounded-xl p-4 border border-white/8">
              <p className="text-xs text-white/70 mb-4">Conversations this week</p>
              <div className="flex items-end gap-2 h-24">
                {BAR_DATA.map((val, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    />
                    <span className="text-[11px] text-white/70">{DAYS[i]}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* BANT tier donut */}
            <div className="glass-1 rounded-xl p-4 border border-white/8">
              <p className="text-xs text-white/70 mb-3">Lead tier distribution</p>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={t.label} className="flex items-center gap-2 text-xs text-white/70">
                      ...
                      {t.label}
                      <span className="ml-auto text-white/70">{t.pct}</span>
                    </div>
                  ))}
                </div>
              </div>
